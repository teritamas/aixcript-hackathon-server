...
class BaseContract:
    ...
    def __init__(
        ...
        if self.network.is_connected():
            logger.info(f"Connect Complete !!. network: {provider_network_url}")

        self.contract = self.network.eth.contract(
            address=self.convert_checksum_address(contract_address),
            abi=self._load_abi(api_json_path),
        )
        logger.info(
            f"Owner Wallet Address: {contract_owner.address}, 残高: {Web3.from_wei(self.network.eth.get_balance(contract_owner.address),'ether')} ether"  # NOQA
        )
        logger.info(
